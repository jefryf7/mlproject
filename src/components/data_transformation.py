...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        ...
        try:
            ...
            preproseccing_obj = self.get_data_transformer_object()
            ...
            logging.info(
                f'Applying preproceccing object on training dataframe and testing dataframe'
            )
            
            input_feature_train_arr = preproseccing_obj.fit_transform(input_feature_train_df)
            input_feature_test_arr = preproseccing_obj.transform(input_feature_test_df)
            
            train_arr = np.c_[input_feature_train_arr,np.array(target_feature_train_df)]
            test_arr = np.c_[input_feature_test_arr,np.array(target_feature_test_df)]
            
            logging.info(f'saved preproceccing object.')
            
            save_object(
                file_path = self.data_transformation_config.preprocessor_obj_file_path,
                obj = preproseccing_obj
            )
